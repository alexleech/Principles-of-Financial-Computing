...
def MCLSQ(data):
	...
	for i in range(k):
		...
		secondParam = secondParam_without_norm*np.random.normal(0,1.0,n)
		#print secondParam
		#print S*np.exp(np.cumsum(firstParam+secondParam))
		StockPath[i, :] = S*np.exp(np.cumsum(firstParam+secondParam))
	#print StockPath
	...
	if len(sys.argv) > 1:
		...
		for test in data:
			...
			for i in range(times):
				...
				#toc = time.clock()
				#print toc-tic
			print "mean = "+str(np.mean(ans))
			print "std  = "+str(np.std(ans))
		#print "finished!"
	else:
